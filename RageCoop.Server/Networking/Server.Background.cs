    ...
    public partial class Server
    {
        ...
        private void KickAssholes()
        {
            foreach (var c in ClientsByNetHandle.Values.ToArray())
            {
                ...
                {
                    c.Kick("You were kicked: Spamming entities.");
                    API.SendChatMessage($"{c.Username} was kicked for spamming");
                }
                else if (Settings.KickGodMode && c.Player.IsInvincible)
                {
                    c.Kick("You were kicked: Godmode");
                    API.SendChatMessage($"{c.Username} was kicked: Godmode");
                }
            }
        }
    }
